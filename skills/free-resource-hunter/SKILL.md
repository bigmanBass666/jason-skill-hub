...
**输出格式要求**：
- 每行必须是独立一行（换行符分隔），标题前后空行，`├`/`•`/表格行都是独立段落——Markdown 渲染依赖换行，不换行会导致所有内容挤成一团
...
- **重点标注日吞吐量**（tokens/day）而非仅标注请求次数——高频用户一天可能消耗 1-2 亿 token，"200 请求/天"这种限制形同虚设
- **区分同一平台内不同模型的额度差异**：比如 OpenRouter 上某些免费模型限 200 次/天，但另一些（如 owl-alpha）可达 1000 次/天，不能一概而论
...
**基线完全不可用**（GitHub MCP + gh CLI 都失败）：
- 降级为「无基线模式」：仍然执行完整的情报搜索流程（第 1 步），但跳过增量对比（第 2 步）
- 输出时在简报顶部标注「⚠️ 未与基线对比——本次扫描报告所有发现均为绝对新增判断，可能包含已知信息」
- 搜索范围适当放宽（不依赖基线过滤），让用户自行判断
- 不阻止用户使用——基线不可用 ≠ skill 不可用，只是失去了增量对比能力
- 下次扫描时重新尝试读取基线
